...
internal class TossEffCommand : Command
    ...
    {
        public SetGoldCommand() : base("gold", 6) { }
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (string.IsNullOrEmpty(args[0]))
            {
    ...
    {
        public SetTokensCommand() : base("tokens", 6) { }
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (string.IsNullOrEmpty(args[0]))
            {
    ...
    {
        public SetFameCommand() : base("fame", 6) { }
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (string.IsNullOrEmpty(args[0]))
            {
    ...
    internal class ArenaCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            Entity entity = Entity.Resolve(player.Manager, 0x0742);
            World we = player.Manager.GetWorld(player.Owner.Id); //can't use Owner here, as it goes out of scope
            int TimeoutTime = player.Manager.GameData.Portals[0x0742].TimeoutTime;
            string DungName = player.Manager.GameData.Portals[0x0742].DungeonName;
