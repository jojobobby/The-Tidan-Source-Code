...
namespace wServer.realm.entities.player
{
    ...
    public partial class Player : Character, IContainer, IPlayer
    {
        ...
        private bool CheckHalfHPBerserk()
        {
            for (var i = 0; i < 4; i++)
            {
                var item = Inventory[i];
        ...
        public void Damage(int dmg, Entity chr)
        {
            try
            {
                ...
                if (CheckHalfHPArmored() && HP <= MaxHp / 3)
                {
                ...
                if (CheckHalfHPBerserk() && HP <= MaxHp / 2)
                {
                    ApplyConditionEffect(new ConditionEffect
                    {
        ...
        protected override void ExportStats(IDictionary<StatsType, object> stats)
        {
            ...
            if (Boost != null)
            {
                ...
                stats[StatsType.Defense] = Stats[3] + NearbyDefBoost() + Boost[3];
                stats[StatsType.Speed] = Stats[4] + Boost[4];
                stats[StatsType.Vitality] = Stats[5] + MPabovehalf() + Boost[5];
                stats[StatsType.Wisdom] = Stats[6] + Boost[6];
                stats[StatsType.Dexterity] = Stats[7] + HPabovehalf() + Boost[7];

                stats[StatsType.HPBoost] = Boost[0];
                stats[StatsType.MPBoost] = Boost[1];
                stats[StatsType.AttackBonus] = Boost[2];
                stats[StatsType.DefenseBonus] = Boost[3] + NearbyDefBoost();
                stats[StatsType.SpeedBonus] = Boost[4];
                stats[StatsType.VitalityBonus] = Boost[5] + MPabovehalf();
                stats[StatsType.WisdomBonus] = Boost[6];
                stats[StatsType.DexterityBonus] = Boost[7] + HPabovehalf();
            }
        ...
        {
            if (CheckHPabovehalf() && HP >= MaxHp / 10)
            {
                return StatsType.Dexterity + 4;
            }
            else
            {
                return 0;
